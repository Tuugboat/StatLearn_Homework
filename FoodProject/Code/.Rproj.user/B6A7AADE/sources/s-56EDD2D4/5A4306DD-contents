...
Sys.glob(here("Data/FDBRaw/id*.json")) %>% foreach(CurFile=., .combine="rbind") %do% {
  ...
  tryCatch(
    {
      jsonlite::fromJSON(txt=CurFile) %$%
        PullMolecules(entity_alias_readable, entity_id, molecules) # See definition above
    },
  ...
  # this means that each alias, with each id, for each molecule, has exactly one entry
  # Without this, pivot_wider sets some results as lists of 1s
  group_by(alias, ID, common_name) %>%
  summarize(IndVal = 1) %>%
